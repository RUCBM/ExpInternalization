...
# Pin distributed comms to the loopback interface (helps single-node vLLM).
export GLOO_SOCKET_IFNAME=lo
export NCCL_SOCKET_IFNAME=lo
# If startup still fails, try also:
# export VLLM_NO_DIST=1
# export VLLM_DISABLE_DISTRIBUTED=1
...
# Collect background server PIDs.
server_pids=$(jobs -p)
