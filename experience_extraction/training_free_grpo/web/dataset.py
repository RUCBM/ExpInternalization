...
# Directory holding the bundled data files (WebWalkerQA.jsonl). Resolved relative
# to this file so the package is self-contained regardless of the working dir.
...
def load_AFM_web_RL(split="train"):
    # Prefer a locally bundled parquet if present; otherwise pull from the Hub.
    # The parquet is not shipped with this repo to keep it lightweight.
    local_parquet = os.path.join(_DATA_DIR, "AFM-WebAgent-RL-Dataset.parquet")
    if os.path.exists(local_parquet):
        dataset = load_dataset(_DATA_DIR, data_files="AFM-WebAgent-RL-Dataset.parquet", split=split)
    else:
...
def load_WebWalkerQA(split="main"):
    dataset = load_dataset(
        _DATA_DIR,                       # bundled data directory
        data_files="WebWalkerQA.jsonl",  # bundled file
    )
    # load_dataset may return a DatasetDict; pick the first split if so.
