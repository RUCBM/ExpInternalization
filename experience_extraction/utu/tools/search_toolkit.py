...
# Anti-cheating: blocked URL patterns for web_qa/crawl
...
def _is_blocked_url(url: str) -> bool:
    """Check if a URL should be blocked to prevent data leakage."""
    if not url:
        return False
...
class SearchToolkit(AsyncBaseToolkit):
    ...
    async def web_qa(self, url: str, query: str) -> str:
        ...
        if _is_blocked_url(url):
            return (
                "Blocked: This URL belongs to a restricted site (e.g., HuggingFace datasets/spaces). "
                "Please do not attempt to access dataset or benchmark sources directly."
            )
        content = await self.crawl_engine.crawl(url)
        # truncate content to avoid exceeding LLM context length
        max_chars = self.summary_token_limit * 4  # ~4 chars per token
        if len(content) > max_chars:
            content = content[:max_chars] + "\n\n[Content truncated due to length limit]"
            logger.info(f"[tool] web_qa: content truncated to {max_chars} chars")
        query = (
