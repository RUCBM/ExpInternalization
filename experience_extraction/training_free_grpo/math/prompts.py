...
6. The total experience pool MUST NOT exceed {max_pool_size} entries. If the current pool plus additions would exceed this limit, you MUST merge similar experiences or remove low-utility ones to stay within the limit. This is a HARD CONSTRAINT.
...
EXPERIENCE_CONSOLIDATION_TEMPLATE = """The current experience pool has exceeded the maximum allowed size of {max_pool_size}. You must consolidate it down to at most {max_pool_size} experiences.

Current experience pool ({current_size} experiences, exceeds limit of {max_pool_size}):

{experiences_list}

Consolidation rules:
1. Merge highly similar or overlapping experiences into more general, unified forms
2. Remove redundant experiences that are subsumed by others
3. Remove overly specific experiences with limited applicability
4. Keep the most general, high-utility, broadly applicable experiences
5. Each experience should be concise (one sentence, no more than 32 words) and general
6. Preserve the diversity of topics — do not over-merge distinct categories
