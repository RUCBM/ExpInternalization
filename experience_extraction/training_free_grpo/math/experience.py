...
from training_free_grpo.math.prompts import (
    ...
    EXPERIENCE_CONSOLIDATION_TEMPLATE,
)
...
class ExperienceUpdater:
    ...
    def run(self, rollouts, experiences, save_dir, max_workers=16, given_ground_truth=True, only_partial_correct=True, max_pool_size=50):
    ...
    def _single_rollout_summary(
        ...
        """
        Step 1: summarize a single rollout trajectory.
        - Condense the agent's full solving process into a concise text description.
        - Focus on the reasoning approach, key steps, and success/failure causes.
        """
        ...
        for rollouts in problems_to_rollouts.values():
            if given_ground_truth and only_partial_correct:
                # only for those partially correct (avg reward strictly between 0 and 1)
    ...
    def _consolidate_experiences(self, experiences, max_pool_size, max_retries=3):
        ...
        for _ in range(max_retries):
            try:
                ...
                response = self.llm.chat(
                    EXPERIENCE_CONSOLIDATION_TEMPLATE.format(
                        max_pool_size=max_pool_size,
                        current_size=len(experiences),
                        experiences_list=formatted_exps
                    )
                )
        ...
        print(f"Warning: consolidation failed after {max_retries} retries, keeping {len(experiences)} experiences")
        return experiences
